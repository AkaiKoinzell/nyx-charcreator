...
import { VStack, Stack, Container, Skeleton, Alert, AlertIcon, Select, CloseButton, InputGroup, InputLeftAddon, Input, useStatStyles, InputRightElement, Spinner, Center } from "@chakra-ui/react";
...
import { useItemsPrefetch, useSearchItemsQuery } from "../../services/item"
...
export const AllItemsPage = () => {
    ...
    return (<>
        ...
        {!!page && <VStack alignItems="left">
            ...
            {pageStatus === QueryStatus.fulfilled && page.entities.map(it => <ItemDisplay key={it.name} item={it} onLabelClick={id => {
                onLabelChange(loadedLabels?.find(it => it.id === id))
            }} />)}
